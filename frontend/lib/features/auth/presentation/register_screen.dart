...
class _RegisterScreenState extends State<RegisterScreen> {
  ...
  bool _showOtpForm =
      false; // Trạng thái hiển thị: false = Form đăng ký, true = Form nhập mã OTP
  int _otpCountdown = 0; // Đếm ngược gửi lại mã OTP (giây)
  Timer? _timer; // Đối tượng đếm giờ định kỳ
  bool _isSendingOtp =
      false; // Trạng thái đang gửi yêu cầu OTP lên server (hiển thị Loading)
  ...
  Future<void> _sendOtpAndGoToVerification() async {
    if (!_formKey.currentState!.validate())
      return; // Xác thực các trường dữ liệu
    setState(() {
      _isSendingOtp = true;
    });
    try {
