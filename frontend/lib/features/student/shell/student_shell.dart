...
class _StudentShellState extends State<StudentShell> {
  ...
  Future<void> _initConnectivity() async {
    ...
    _connectivitySubscription = conn.onConnectivityChanged.listen(_onConnectivityChanged);
  ...
  void _onConnectivityChanged(List<ConnectivityResult> result) {
    ...
    if (justReconnected && mounted) {
      ScaffoldMessenger.of(context)
        ...
        ..showSnackBar(
          const SnackBar(
            content: Text('Đã kết nối lại mạng. Đang kiểm tra dữ liệu chờ đồng bộ...'),
            duration: Duration(seconds: 2),
          ),
        );
    }

    // Always try to sync; schedule after current frame to avoid build-phase issues
  ...
  Future<void> _syncPending({bool isReconnect = false}) async {
    ...
    _syncing = true;
    try {
      // Wait for network to stabilize after reconnect
      if (isReconnect) await Future.delayed(const Duration(milliseconds: 1500));
      if (!mounted) return;

      debugPrint('[SYNC] Starting sync for user: $userId, isReconnect: $isReconnect');
      final summary = await OfflineSyncService.instance.sync(userId);
      debugPrint('[SYNC] Result: attempts=${summary.attempts}, progress=${summary.progress}, details=${summary.attemptDetails.length}');
      if (!mounted) return;
      ...
      if (hasData) {
        ...
        ScaffoldMessenger.of(context)
          ..hideCurrentSnackBar()
          ..showSnackBar(
            const SnackBar(content: Text('Đã có mạng. Không có dữ liệu chờ đồng bộ.')),
          );
      }
    } catch (e) {
      ...
      if (isReconnect) {
        ScaffoldMessenger.of(context)
          ...
          ..showSnackBar(
            const SnackBar(
              content: Text('Có mạng nhưng chưa đồng bộ được. Dữ liệu vẫn được giữ lại.'),
  ...
  Future<void> _showSyncSuccessDialog(OfflineSyncSummary summary) async {
    if (!mounted) return;
    debugPrint('[SYNC] _showSyncSuccessDialog called, attempts=${summary.attempts}');
    await showDialog<void>(
      ...
      builder: (ctx) {
        ...
        // Build detail rows as plain widgets (no ListView to avoid layout errors)
        final detailWidgets = summary.attemptDetails.asMap().entries.map((entry) {
              ...
              child: Column(
                ...
                children: [
                  if (summary.attempts > 0) ...[
                    Text('Hệ thống đã chấm điểm ${summary.attempts} bài làm ngoại tuyến.'),
                  ...
                  if (detailWidgets.isNotEmpty) ...[
                    ...
                    Row(
                      children: [
                        ...
                        Text('Đạt: ${summary.passedCount} bài',
                            style: const TextStyle(fontSize: 13)),
                        const SizedBox(width: 16),
                        Icon(Icons.cancel, color: colors.error, size: 15),
                        const SizedBox(width: 4),
                        Text('Chưa đạt: ${summary.failedCount} bài',
                            style: const TextStyle(fontSize: 13)),
                      ],
                    ),
                  ...
                  if (summary.progress > 0) ...[
                    ...
                    Text('📚 Đồng bộ ${summary.progress} tiến độ bài học.',
                        style: const TextStyle(fontSize: 13)),
                  ],
                  const SizedBox(height: 8),
                ],
      ...
      bottomNavigationBar: Container(
        ...
        child: NavigationBar(
          ...
          destinations: const [
            ...
            NavigationDestination(icon: Icon(Icons.leaderboard_outlined), selectedIcon: Icon(Icons.leaderboard_rounded), label: 'Xếp hạng'),
