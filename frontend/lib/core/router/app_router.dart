...
GoRouter createAppRouter(AuthProvider authProvider) {
  // Router lắng nghe AuthProvider để tự chạy lại redirect khi login/logout đổi trạng thái.
  return GoRouter(
    navigatorKey: _rootNavigator,
    initialLocation: '/splash',
    refreshListenable: authProvider,
    redirect: (BuildContext context, GoRouterState state) {
      // Đọc trạng thái xác thực và role hiện tại để bảo vệ route ở phía UI.
      final isLoggedIn = authProvider.isAuthenticated;
      final role = authProvider.currentUser?.role ?? '';
      final location = state.uri.toString();

      if (!isLoggedIn) {
        // Khách chỉ được ở nhóm màn hình xác thực; route khác bị đưa về login.
        if (location == '/splash' ||
            location == '/login' ||
            location == '/register' ||
      ...
      if (role == 'Admin') {
        // Admin không được đi vào cây route /student.
      ...
      if (role == 'Student') {
        // Student không được đi vào cây route /admin.
    ...
    routes: [
      ...
      StatefulShellRoute.indexedStack(
        // IndexedStack giữ state của mỗi tab khi Student chuyển qua lại.
        ...
        branches: [
              ...
              GoRoute(
                ...
                routes: [
                  GoRoute(
                    path: ':id',
                    builder: (_, state) =>
                        LessonsScreen(chapterId: state.pathParameters['id']!),
                  ),
                ],
              ),
              GoRoute(
          ...
          StatefulShellBranch(
            routes: [
              // Bảng xếp hạng là tab chính thứ ba, thay cho tab Bảng tin trước đây.
              GoRoute(
                path: '/student/leaderboard',
                builder: (_, __) => const LeaderboardScreen(),
              ),
              // Route dashboard cũ vẫn được giữ trong cùng branch để tương thích link cũ.
              GoRoute(
        ...
        branches: [
          ...
          StatefulShellBranch(
            routes: [
              GoRoute(
                path: '/admin/chapters',
                builder: (_, __) => const AdminChaptersScreen(),
                routes: [
                  GoRoute(
                    path: ':chapterId/lessons',
          ...
          StatefulShellBranch(
            routes: [
              GoRoute(
                path: '/admin/badges',
                builder: (_, __) => const AdminBadgesScreen(),
              ),
            ],
          ),
          StatefulShellBranch(
              ...
              GoRoute(
                ...
                routes: [
                  GoRoute(
                    path: ':id/history',
                    builder: (_, state) =>
                        UserHistoryScreen(userId: state.pathParameters['id']!),
                  ),
                ],
              ),
            ],
          ...
          StatefulShellBranch(
            routes: [
              GoRoute(
                path: '/admin/reward-policies',
                builder: (_, __) => const AdminRewardPoliciesScreen(),
              ),
            ],
          ),
          StatefulShellBranch(
