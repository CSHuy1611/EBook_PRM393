...
public partial class AuthService
{
    public async Task SendOtpAsync(OtpRequest request)
    {
        ...
        _logger.LogInformation("Generated OTP for {Email}: {Otp}", email, otp);

        // Send email
        var subject = "Math-IBook: Mã xác thực OTP đăng ký tài khoản";
        var body = $@"
            <div style='font-family: Arial, sans-serif; max-width: 600px; margin: auto; padding: 20px; border: 1px solid #ddd; border-radius: 10px;'>
                ...
                <div style='padding: 10px 0;'>
                    <p>Xin chào,</p>
                    <p>Bạn đang thực hiện đăng ký tài khoản trên ứng dụng <b>Math-IBook</b>. Dưới đây là mã xác thực OTP của bạn:</p>
                    ...
                    <p style='color: #ff9800; font-size: 13px;'><i>Mã OTP này có hiệu lực trong vòng 5 phút. Vui lòng không chia sẻ mã này cho bất kỳ ai.</i></p>
                </div>
                <hr style='border: none; border-top: 1px solid #eee;' />
    ...
    private bool VerifyOtp(string email, string enteredOtp)
    {
        var isDevelopment = _configuration["ASPNETCORE_ENVIRONMENT"] == "Development"
            || Environment.GetEnvironmentVariable("ASPNETCORE_ENVIRONMENT") == "Development";

        if (isDevelopment && enteredOtp == "123456")
        {
            _logger.LogInformation("Development mode: verified OTP via default code '123456' for {Email}", email);
            return true;
        }

        var cacheKey = $"otp:{NormalizeEmail(email)}";
